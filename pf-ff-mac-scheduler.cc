    ...
    const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    // Реализованный планировщик Proportional Fair, используемый в ns3
    int i = 0;
    uint16_t rbLen = 1;
    rbgAllocationMap.resize(m_cschedCellConfig.m_ulBandwidth, 0);
    do
    {
        NS_LOG_INFO(this << " ALLOCATION for RBG " << i << " of " << m_cschedCellConfig.m_ulBandwidth);
        
        if (rbMap.at(i) == false)
        {
            std::map<uint16_t, pfsFlowPerf_t>::iterator itFlow;
            std::map<uint16_t, pfsFlowPerf_t>::iterator itMax = m_flowStatsUl.end();
            double rcqiMax = 0.0;
            for (itFlow = m_flowStatsUl.begin(); itFlow != m_flowStatsUl.end(); itFlow++)
            {
                if ((m_ffrSapProvider->IsUlRbgAvailableForUe(i, (*itFlow).first)) == false)
                {
                    continue;
                }
                ...
                itCqi = m_ueCqi.find((*itFlow).first);
                std::map<uint16_t, uint8_t>::iterator itTxMode;
                        ...
                        (cqi2 > 0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
                {
                    ...
                    for (uint8_t j = 0; j < nLayer; j++)
                    {
                        ...
                        else
                        {
                            ...
                            NS_ABORT_MSG_IF((*itCqi).second.size() == 0,
                                            "CQI of RNTI = " << (*itMax).first << " has expired");
                            double minSinr = (*itCqi).second.at(i);
                            if (minSinr == NO_SINR)
                            {
                                // minSinr = EstimateUlSinr((*it).first, uldci.m_rbStart);
                                minSinr = EstimateUlSinr((*itMax).first, i);
                            }
                            ...
                            for (uint16_t j = i; j < size; j++)
                            {
                                ...
                                if (sinr == NO_SINR)
                                {
                                    ...
                                    sinr = EstimateUlSinr((*itMax).first, j);
                                }
                                if (sinr < minSinr)
                                {
                                    minSinr = sinr;
                            ...
                            if (cqi == 0)
                            {
                                ...
                                NS_LOG_DEBUG(this << " UE discarded for CQI = 0, RNTI " << (*itMax).first);
                        ...
                        achievableRate += ((m_amc->GetUlTbSizeFromMcs(mcs, rbLen) / 8) /
                                            0.001); // = TB size / TTI
                    }

                    double rcqi = achievableRate / (*itFlow).second.lastAveragedThroughput;
                    NS_LOG_INFO(this << " RNTI " << (*itFlow).first << " MCS " << (uint32_t)mcs
                                        << " achievableRate " << achievableRate << " avgThr "
                                        << (*itFlow).second.lastAveragedThroughput << " RCQI "
                                        << rcqi);
            ...
            {
                rbMap.at(i) = true;
                rbgAllocationMap.at(i) = (*itMax).first;
                // if (itMap == allocationMap.end())
                // {
                //     // insert new element
                //     std::vector<uint16_t> tempMap;
        ...
        i++;
    } /*end for RBGs */ while (i < m_cschedCellConfig.m_ulBandwidth); 
    ...
    // std::map<uint16_t, double>::iterator itBet;
    ...
    //     for (itBet = estAveThr.begin(); itBet != estAveThr.end(); itBet++)
    //     {
    //         double metric = 1 / (*itBet).second;
    //         if (metric > metricMax)
    //         {
    //             metricMax = metric;
    //             itMax = itBet;
    //         }
    //         rbgPerRntiLog.insert(std::pair<uint16_t, int>((*itBet).first, 1));
    //     }
    ...
    //             std::vector<uint16_t> tempMap;
    //             std::copy_if(rbgAllocationMap.begin(), rbgAllocationMap.end(), std::back_inserter(Map), [&](uint16_t UE){ return UE == (*itMax).first; });
    //             std::vector<uint16_t>::iterator itMap = Map.begin();
    //             rbgAllocationMap.at(i) = (*itMax).first;
    //             // itMap = rbgAllocationMap.find((*itMax).first);
    //             if (Map.size() == 0)
    //             {
    //                 tempMap.push_back(i);
                    
    //                 // rbgAllocationMap.insert(
    //                 //     std::pair<uint16_t, std::vector<uint16_t>>((*itMax).first, tempMap));
    //             }
    //             else
    //             {
    ...
    //             std::map<uint16_t, std::vector<double>>::iterator itCqi;
    //             itCqi = m_ueCqi.find((*itMax).first);
    //             std::map<uint16_t, uint8_t>::iterator itTxMode;
    ...
    //                     rbLen = rbgPerRntiLog[(*itMax).first];
    //                     // rbLen = (i + rbgPerRntiLog[(*itMax).first] < m_cschedCellConfig.m_ulBandwidth) ? rbgPerRntiLog[(*itMax).first] : 1;
    //                     uint16_t size = ((i + rbgPerRntiLog[(*itMax).first]) <= m_cschedCellConfig.m_ulBandwidth) ? i + rbLen : m_cschedCellConfig.m_ulBandwidth - rbLen;
    //                     // uint16_t size = i + rbLen;
    //                     for (uint16_t j = i; j < size; j++)
    //                     {
    //                         double sinr = (*itCqi).second.at(j);
    ...
    //             std::map<uint16_t, pfsFlowPerf_t>::iterator itPastAveThr;
    //             itPastAveThr = m_flowStatsUl.find((*itMax).first);
    //             uint32_t bytesTxed = 0;
    //             for (uint8_t j = 0; j < nLayer; j++)
    //             {
    //                 int tbSize =
    //                     (m_amc->GetUlTbSizeFromMcs(mcs, (*itRbgPerRntiLog).second) /
    //                     8); // (size of TB in bytes according to table 7.1.7.2.1-1 of 36.213)
    //                 bytesTxed += tbSize;
    //             }
    ...
    //     } while (i < m_cschedCellConfig.m_ulBandwidth); // end for RBGs
    // }// end if estAveThr
    ...
    for (itStats = m_flowStatsUl.begin(); itStats != m_flowStatsUl.end(); itStats++)
    {
        ...
        // reset TTI stats of users
        (*itStats).second.lastTtiBytesTrasmitted = 0;
        std::copy_if(rbgAllocationMap.begin(), rbgAllocationMap.end(), std::back_inserter(tempVectorOfRBs), [&](uint16_t UE){ return UE == (*itStats).first; });
        allocationMap.insert(
            std::pair<uint16_t, std::vector<uint16_t>>((*itStats).first, tempVectorOfRBs));
    }
    ...
    do
    {
        ...
        for (uint8_t j = 0; j < nLayer; j++)
        {
            ...
            uldci.m_mcs = m_amc->GetMcsFromCqi(worstCqi.at(j));
        ...
        std::map<uint16_t, pfsFlowPerf_t>::iterator itStatsUl;
        itStatsUl = m_flowStatsUl.find((*itMap).first);
        if (itStatsUl != m_flowStatsUl.end())
        {
            (*itStatsUl).second.lastTtiBytesTrasmitted = bytesTxed;
            NS_LOG_INFO(this << " UE total bytes txed " << (*itStatsUl).second.lastTtiBytesTrasmitted);
        }
        else
        {
            NS_FATAL_ERROR(this << " No Stats for this allocated UE");
    ...
    // Планировщик Proportional Fair в ns3
    // do
    // {
    //     std::set<uint16_t>::iterator itRnti = rntiAllocated.find((*it).first);
    //     if ((itRnti != rntiAllocated.end()) || ((*it).second == 0))
    ...
    // Планировщик Proportional Fair, используемый в ns3
    // do
    // {
    //     std::set<uint16_t>::iterator itRnti = rntiAllocated.find((*it).first);
    //     if ((itRnti != rntiAllocated.end()) || ((*it).second == 0))
