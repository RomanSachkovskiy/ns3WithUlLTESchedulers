    ...
    const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    // Реализованный планировщик Td Blind Equal Throughput
    std::map<uint16_t, tdbetsFlowPerf_t>::iterator itFlow;
    std::map<uint16_t, tdbetsFlowPerf_t>::iterator itMax = m_flowStatsUl.end();
    double metricMax = 0.0;
    for (itFlow = m_flowStatsUl.begin(); itFlow != m_flowStatsUl.end(); itFlow++)
    {
        ...
        double metric = 1 / (*itFlow).second.lastAveragedThroughput;
    ...
    do
    {
        ...
        std::map<uint16_t, tdbetsFlowPerf_t>::iterator itStatsUl;
    ...
    //     std::cout << "FRAME: " << (params.m_sfnSf >> 4) << ", subFrame: " << (0xF & params.m_sfnSf) << std::endl;
    //     uldci.m_tbSize = (m_amc->GetUlTbSizeFromMcs(uldci.m_mcs, rbPerFlow) / 8);
    //     std::cout << "RNTI: " << (*it).first<< std::endl;
    //     std::cout << "BLOCKS: " << rbPerFlow << std::endl;
    //     std::cout << "CMS: " << uldci.m_mcs << std::endl;
    //     std::cout << std::endl;
    //     UpdateUlRlcBufferInfo(uldci.m_rnti, uldci.m_tbSize);
    //     uldci.m_ndi = 1;
    //     uldci.m_cceIndex = 0;
